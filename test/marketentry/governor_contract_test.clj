  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Palikir Fabrication LLC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Palikir Fabrication LLC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest citizen-bidder-preference-mismatch-is-held-and-unoverridable
  (testing "declared :citizen-bidder? true but workforce/materials composition fails eligibility -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:citizen-bidder-preference-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest fsm-tax-filing-unverified-is-held-and-unoverridable
  (testing "unverified FSM wage/gross-revenue tax filing when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:fsm-tax-filing-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
