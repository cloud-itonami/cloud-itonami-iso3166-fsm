...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (FSM, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up citizen-bidder-preference-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (citizen-bidder-preference-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up fsm-tax-filing-unverified) ==")
    ...
    (println "== filing/submit eng-5 (fsm-tax-filing-unverified -> HARD hold) ==")
